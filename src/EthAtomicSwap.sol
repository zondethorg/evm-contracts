...
contract EthAtomicSwap is Ownable, Pausable, ReentrancyGuard {
    ...
    function lock(
        ...
        emit Locked(
            swapID, hashSecret, assetLocked, value, recipient, desiredAssetRaw, desiredAmount, expiryTs, msg.sender
        );
    }
    ...
    function claim(bytes32 swapID, bytes calldata secret) external whenNotPaused nonReentrant swapExists(swapID) {
        ...
        require(keccak256(abi.encodePacked(s.locker, hashSecret, s.recipient)) == swapID, "AS: wrong secret");
     ...
     */
    function previewSwapID(address locker, bytes32 hashSecret, address recipient) external pure returns (bytes32) {
